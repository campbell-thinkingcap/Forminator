...
JSON Schema allows additional properties by convention. We use an `x-` prefix for AI-specific hints: `x-prompt`, `x-source`, `x-options-source`, `x-options-preview`, `x-order`, `x-group`, `x-hint`, `x-depends-on`, and `x-widget`.

**The normative definition of the `x-` vocabulary — values, defaults, widget rules, and the runtime precedence rules every runtime must implement — lives in [docs/SCHEMA-AUTHORING-STANDARD.md](docs/SCHEMA-AUTHORING-STANDARD.md).** That document is canonical; this guide is the introduction. Lint your schema against it with `POST /api/schema/lint`.
